...
context( 'mlib', function()
	...
	context( 'util', function()
		...
		context( 'removeDuplicatePairs', function()
			...
			test( 'Removes duplicate pairs from the table when pairs are duplicated.', function()
		...
		context( 'removeDuplicatePointsFlat', function()
			...
			test( 'Removes duplicate pairs from the table when pairs are duplicated.', function()
				...
				assert_multiple_fuzzy_equal( _.util.removeDuplicatePointsFlat( { -0.6, -2.1, -0.6, -2, -0.6, -2 } ), { -0.6, -2.1, -0.6, -2 } )
			end )
		end )
    ...
    context( 'point', function()
        ...
        context( 'polarToCartesian', function()
            test( 'Convert polar coordinate to cartesian coordinate', function()
                assert_multiple_fuzzy_equal( { _.point.polarToCartesian( 10, math.pi / 2 ) }, { 0, 10 } )
                assert_multiple_fuzzy_equal( { _.point.polarToCartesian( math.sqrt( 2 ), math.pi / 4 ) }, { 1, 1 } )
            end )

            test( 'Uses absolute angle', function()
                assert_multiple_fuzzy_equal( { _.point.polarToCartesian( math.sqrt( 2 ), 5 * math.pi / 4 ) }, { -1, -1 } )
            end )
        ...
        context( 'cartesianToPolar', function()
            test( 'Convert cartesian coordinate to polar coordinate', function()
                assert_multiple_fuzzy_equal( { _.point.cartesianToPolar( 0, 10 ) }, { 10, math.pi / 2 } )
                assert_multiple_fuzzy_equal( { _.point.cartesianToPolar( 1, 1 ) }, { math.sqrt( 2 ), math.pi / 4 } )
            end )

            test( 'Uses absolute angle', function()
                assert_multiple_fuzzy_equal( { _.point.cartesianToPolar( -1, -1 ) }, { math.sqrt( 2 ), 5 * math.pi / 4 } )
            end )

            test( 'Offset', function()
                assert_multiple_fuzzy_equal( { _.point.cartesianToPolar( 5, 13, 5, 3 ) }, { 10, math.pi / 2 } )
                assert_multiple_fuzzy_equal( { _.point.cartesianToPolar( 10, 11, 9, 10 ) }, { math.sqrt( 2 ), math.pi / 4 } )
                assert_multiple_fuzzy_equal( { _.point.cartesianToPolar( -4.5, 1, -3.5, 2 ) }, { math.sqrt( 2 ), 5 * math.pi / 4 } )
            end )
        end )
    end )
